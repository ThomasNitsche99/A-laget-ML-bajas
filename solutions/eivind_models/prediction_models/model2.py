...
# Preparing X and y for SOG, COG, and ROT
X_train = ais_data[feature_cols]
y_train_sog = ais_data['SOG']
y_train_cog = ais_data['COG']
y_train_rot = ais_data['ROT']

# Train models for SOG, COG, and ROT
model_sog = RandomForestRegressor(random_state=42)
model_sog.fit(X_train, y_train_sog)

model_cog = RandomForestRegressor(random_state=42)
model_cog.fit(X_train, y_train_cog)

model_rot = RandomForestRegressor(random_state=42)
model_rot.fit(X_train, y_train_rot)
...
# Step 1: Predict SOG, COG, and ROT in the validation set
y_sog_pred = model_sog.predict(X_valid)
y_cog_pred = model_cog.predict(X_valid)
y_rot_pred = model_rot.predict(X_valid)

# Combine SOG, COG, and ROT predictions with actual values
predictions['SOG_actual'] = y_train_sog.iloc[split_index:].values
predictions['COG_actual'] = y_train_cog.iloc[split_index:].values
predictions['ROT_actual'] = y_train_rot.iloc[split_index:].values
predictions['SOG_pred'] = y_sog_pred
predictions['COG_pred'] = y_cog_pred
predictions['ROT_pred'] = y_rot_pred

# Display SOG, COG, and ROT errors
predictions['SOG_error'] = predictions['SOG_actual'] - predictions['SOG_pred']
predictions['COG_error'] = predictions['COG_actual'] - predictions['COG_pred']
predictions['ROT_error'] = predictions['ROT_actual'] - predictions['ROT_pred']

print("SOG Mean Error:", predictions['SOG_error'].mean())
print("COG Mean Error:", predictions['COG_error'].mean())
print("ROT Mean Error:", predictions['ROT_error'].mean())
...
# Initialize columns that might not exist in test_data
for col in ['SOG', 'COG', 'ROT', 'HEADING', 'NAVSTATUS_ENCODED']:
    if col not in test_data.columns:
        test_data[col] = np.nan

# Handle TIME_TO_ETA if ETARAW is in the training dataset
if 'ETARAW' in test_data.columns:
    ...
    # If ETARAW is not available, set TIME_TO_ETA to the median from training data
    test_data['TIME_TO_ETA'] = ais_data['TIME_TO_ETA'].median()

# Prepare to create features for prediction
# Handle missing values in numeric features
numeric_features = ['HEADING']
for feature in numeric_features:
    if feature in test_data.columns:
        test_data[feature].fillna(ais_data[feature].median(), inplace=True)

# Predict SOG, COG, and ROT
if 'SOG' not in test_data.columns or test_data['SOG'].isnull().any():
    # Predict SOG using available features
    if all(col in test_data.columns for col in feature_cols if col != 'SOG'):
        test_data['SOG'] = model_sog.predict(test_data[feature_cols])

if 'COG' not in test_data.columns or test_data['COG'].isnull().any():
    # Predict COG using available features
    if all(col in test_data.columns for col in feature_cols if col != 'COG'):
        test_data['COG'] = model_cog.predict(test_data[feature_cols])

if 'ROT' not in test_data.columns or test_data['ROT'].isnull().any():
    # Predict ROT using available features
    if all(col in test_data.columns for col in feature_cols if col != 'ROT'):
        test_data['ROT'] = model_rot.predict(test_data[feature_cols])

# Handle missing values again for SOG, COG, ROT after predictions
for feature in ['SOG', 'COG', 'ROT']:
    if feature in test_data.columns:
        test_data[feature].fillna(ais_data[feature].median(), inplace=True)

# Handle NAVSTATUS_ENCODED
if 'NAVSTATUS_ENCODED' in test_data.columns:
    test_data['NAVSTATUS_ENCODED'] = test_data['NAVSTATUS_ENCODED'].astype(float)  # Temporarily as float
    test_data['NAVSTATUS_ENCODED'].fillna(0, inplace=True)  # Fill NaN values with 0 or an appropriate value
    test_data['NAVSTATUS_ENCODED'] = test_data['NAVSTATUS_ENCODED'].astype(int)  # Convert to int
else:
    # Use the most common NAVSTATUS_ENCODED from training data
    test_data['NAVSTATUS_ENCODED'] = ais_data['NAVSTATUS_ENCODED'].mode()[0]


# Calculate relative bearing if HEADING and COG are available
...
# Categorize ROT into bins if ROT is available
...
output.to_csv('submissions/eivind_submissions/submission_2.csv', index=False)

print("Predictions saved to 'submissions/eivind_submissions/submission_2.csv'")
